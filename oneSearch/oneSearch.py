...
# returns the data for the given ticker from OneSearch
def getTicker(ticker):
    r = requests.get("https://www.onesearch.com/yhs/search;?p=" + ticker)
    data = r.text
    return data

# returns the price of the ticker stock, using data given
def getPrice(data, ticker):
    index = data.find("<div class=\"fin_quotePrice s-price\">")
    if index == -1:
        print("WARNING 1001 - Stock (" + ticker + ") value not found\nUsing 0 for price...\n")
        return 0
    shrunkData = data[index:index+100]
    index1 = shrunkData.find("<div class=\"fin_quotePrice s-price\">")
    index2 = shrunkData.find("</div>")
    shrunkData = shrunkData[index1:index2]
    price = float(shrunkData[shrunkData.find(">")+1:].replace(",", ""))
    return price
...
def getAllPrices(data):
    ...
    for i in range(len(data)):
        if data[i][0] != "Ticker":
            ...
            tickerPrice = getPrice(tickerData, data[i][0])
...
def commandLineParse():
    parser = argparse.ArgumentParser(description="Determines value of stocks using OneSearch")
    #parser.add_argument("-i", "-input", required=True, help="input file name, expects CSV file       [REQUIRED]")
